# Capital Flow — Weekly Report 2026-W34

## Signals fired
- **NVDA (NVIDIA): 21 private flows converge**  _(rule: beneficiary_concentration, strength 21.0)_
- **VRT (Vertiv): 18 private flows converge**  _(rule: beneficiary_concentration, strength 18.0)_
- **ETN (Eaton): 14 private flows converge**  _(rule: beneficiary_concentration, strength 14.0)_
- **GEV (GE Vernova): 11 private flows converge**  _(rule: beneficiary_concentration, strength 11.0)_
- **ai_infrastructure: 10 key allocators converge (30d)**  _(rule: theme_swarm, strength 10.0)_
- **PWR (Quanta Services): 9 private flows converge**  _(rule: beneficiary_concentration, strength 9.0)_
- **ai-applications: 8 key allocators in 30d**  _(rule: sector_swarm, strength 8.0)_
- **ai_applications: 8 key allocators converge (30d)**  _(rule: theme_swarm, strength 8.0)_
- **GOOGL (Alphabet): 8 private flows converge**  _(rule: beneficiary_concentration, strength 8.0)_
- **ai-applications: smart money — Microsoft led, 6 followed in 21d**  _(rule: smart_money_follow, strength 7.0)_
- **TSM (Taiwan Semiconductor): 7 private flows converge**  _(rule: beneficiary_concentration, strength 7.0)_
- **power-energy: 6 key allocators in 30d**  _(rule: sector_swarm, strength 6.0)_
- **energy_for_ai: 6 key allocators converge (30d)**  _(rule: theme_swarm, strength 6.0)_
- **datacenters: smart money — DigitalBridge led, 5 followed in 21d**  _(rule: smart_money_follow, strength 6.0)_
- **ai-data: 5 key allocators in 30d**  _(rule: sector_swarm, strength 5.0)_
- **ai-labs: 5 key allocators in 30d**  _(rule: sector_swarm, strength 5.0)_
- **datacenters: 5 key allocators in 30d**  _(rule: sector_swarm, strength 5.0)_
- **ai-labs: smart money — Elad Gil led, 4 followed in 21d**  _(rule: smart_money_follow, strength 5.0)_
- **defense-tech: smart money — Jeff Bezos led, 4 followed in 21d**  _(rule: smart_money_follow, strength 5.0)_
- **AMZN (Amazon.com Inc): 5 private flows converge**  _(rule: beneficiary_concentration, strength 5.0)_
- **MSFT (Microsoft Corporation): 5 private flows converge**  _(rule: beneficiary_concentration, strength 5.0)_
- **cybersecurity: 4 key allocators in 30d**  _(rule: sector_swarm, strength 4.0)_
- **robotics: 4 key allocators in 30d**  _(rule: sector_swarm, strength 4.0)_
- **cybersecurity/agentic-security: 4 key allocators converge**  _(rule: subsector_swarm, strength 4.0)_
- **power-energy/grid-storage: 4 key allocators converge**  _(rule: subsector_swarm, strength 4.0)_
- **defense_ai: 4 key allocators converge (30d)**  _(rule: theme_swarm, strength 4.0)_
- **robotics_embodiment: 4 key allocators converge (30d)**  _(rule: theme_swarm, strength 4.0)_
- **cybersecurity: smart money — Andreessen Horowitz led, 3 followed in 21d**  _(rule: smart_money_follow, strength 4.0)_
- **MU (Micron): 4 private flows converge**  _(rule: beneficiary_concentration, strength 4.0)_
- **defense-tech: 3 key allocators in 30d**  _(rule: sector_swarm, strength 3.0)_
- **neocloud: 3 key allocators in 30d**  _(rule: sector_swarm, strength 3.0)_
- **semiconductors: 3 key allocators in 30d**  _(rule: sector_swarm, strength 3.0)_
- **ai-applications/drug-discovery-ai: 3 key allocators converge**  _(rule: subsector_swarm, strength 3.0)_
- **ai-applications/enterprise-ai-agents: 3 key allocators converge**  _(rule: subsector_swarm, strength 3.0)_
- **ai-applications/legal-regulatory-ai: 3 key allocators converge**  _(rule: subsector_swarm, strength 3.0)_
- **ai-data/ai-data-platform: 3 key allocators converge**  _(rule: subsector_swarm, strength 3.0)_
- **ai-labs/world-models: 3 key allocators converge**  _(rule: subsector_swarm, strength 3.0)_
- **defense-tech/defense-manufacturing: 3 key allocators converge**  _(rule: subsector_swarm, strength 3.0)_
- **power-energy: smart money — Brookfield led, 2 followed in 21d**  _(rule: smart_money_follow, strength 3.0)_
- **robotics: smart money — Jeff Bezos led, 2 followed in 21d**  _(rule: smart_money_follow, strength 3.0)_
- **Kuwait Oil Company Pipeline JV (Project Peregrine): stealth accumulation — 3 stakes in 90d**  _(rule: stealth_accumulation, strength 3.0)_
- **AVGO (Broadcom): 3 private flows converge**  _(rule: beneficiary_concentration, strength 3.0)_
- **FANUY (FANUC): 3 private flows converge**  _(rule: beneficiary_concentration, strength 3.0)_
- **FLNC (Fluence Energy): 3 private flows converge**  _(rule: beneficiary_concentration, strength 3.0)_
- **SIEGY (Siemens): 3 private flows converge**  _(rule: beneficiary_concentration, strength 3.0)_
- **ai-compute/space-based-ai-compute-infrastructure: 2 key allocators converge**  _(rule: subsector_swarm, strength 2.0)_
- **ai-data/data-platform: 2 key allocators converge**  _(rule: subsector_swarm, strength 2.0)_
- **cybersecurity/military-cyber: 2 key allocators converge**  _(rule: subsector_swarm, strength 2.0)_
- **neocloud/gpu-cloud-financing: 2 key allocators converge**  _(rule: subsector_swarm, strength 2.0)_
- **power-energy/oil-pipeline-lease-leaseback: 2 key allocators converge**  _(rule: subsector_swarm, strength 2.0)_
- **robotics/critical-minerals-automation: 2 key allocators converge**  _(rule: subsector_swarm, strength 2.0)_
- **semiconductors/inference-asic: 2 key allocators converge**  _(rule: subsector_swarm, strength 2.0)_

## New verified events (17)
| Allocator | Target | Sector | Type | Amount | Status | Conf |
|---|---|---|---|--:|---|:-:|
| Saudi PIF | SpaceX | ai-compute | sovereign_investment | $26.4B | verified | A1·100 |
| NVIDIA | SpaceX | ai-compute | corporate_investment | $10.0B | verified | A1·100 |
| Coatue | Databricks | ai-data | funding_round | $5.0B | verified | A1·100 |
| Andreessen Horowitz | Databricks | ai-data | follow_on | $5.0B | verified | A1·100 |
| Thrive Capital | Databricks | ai-data | follow_on | $5.0B | verified | A1·100 |
| Altimeter | Thrive Holdings | ai-applications | minority_stake | $2.0B | verified | A1·100 |
| Sequoia | Form Energy | power-energy | funding_round | $750M | verified | A1·100 |
| Coatue | Form Energy | power-energy | follow_on | $750M | verified | A1·100 |
| Sequoia | Corma | cybersecurity | funding_round | $60M | verified | A1·100 |
| Khosla Ventures | Corma | cybersecurity | funding_round | $60M | verified | A1·100 |
| Coatue | Corma | cybersecurity | funding_round | $60M | verified | A1·100 |
| Andreessen Horowitz | Vals AI | ai-applications | funding_round | $40M | verified | A1·100 |
| Brookfield | Boralex | power-energy | acquisition | — | verified | A1·100 |
| US Government | Performance Drone Works | defense-tech | project_finance | $820M | verified_alpha | A3·82 |
| MGX | Databricks | ai-data | funding_round | — | verified_alpha | A3·82 |
| Blackstone | Databricks | ai-data | funding_round | — | verified_alpha | C3·62 |
| Elad Gil | Cambridge Aerospace | defense-tech | funding_round | — | verified_alpha | C3·62 |

## Top sectors — last 30 days
| Sector | Capital | Deals | Distinct allocators |
|---|--:|--:|--:|
| datacenters | $495.5B | 28 | 15 |
| diversified-pe | $48.6B | 3 | 2 |
| ai-labs | $46.3B | 9 | 7 |
| ai-compute | $36.5B | 6 | 6 |
| nuclear | $18.6B | 3 | 2 |
| ai-data | $15.0B | 5 | 5 |
| power-energy | $10.5B | 13 | 9 |
| ai-applications | $8.9B | 18 | 12 |
| defense-tech | $7.1B | 11 | 6 |
| robotics | $6.3B | 8 | 5 |

## Themes — last 30 days
| Theme | Capital | Deals | Distinct allocators |
|---|--:|--:|--:|
| ai_infrastructure | $515.1B | 47 | 21 |
| frontier_ai | $46.3B | 7 | 5 |
| sovereign_ai | $30.6B | 7 | 6 |
| energy_for_ai | $27.1B | 15 | 9 |
| ai_applications | $18.7B | 20 | 11 |
| defense_ai | $7.1B | 13 | 7 |
| robotics_embodiment | $6.3B | 8 | 5 |
| cyber_security | $180M | 3 | 3 |
| biotech_ai | $0 | 1 | 1 |

## Public beneficiaries mapped (120)
| Ticker | Company | From flow | Confidence |
|---|---|---|:-:|
| GOOGL | Alphabet Inc | Sequoia→Form Energy | medium |
| ETN | Eaton Corporation | Sequoia→Form Energy | low |
| PWR | Quanta Services | Sequoia→Form Energy | low |
| GOOGL | Alphabet Inc | Coatue→Form Energy | medium |
| ETN | Eaton Corporation | Coatue→Form Energy | low |
| PWR | Quanta Services | Coatue→Form Energy | low |
| NVDA | NVIDIA Corporation | Coatue→Databricks | high |
| MSFT | Microsoft Corporation | Coatue→Databricks | high |
| AMZN | Amazon.com Inc | Coatue→Databricks | medium |
| GOOGL | Alphabet Inc | Coatue→Databricks | medium |
| NVDA | NVIDIA Corporation | Andreessen Horowitz→Databricks | high |
| MSFT | Microsoft Corporation | Andreessen Horowitz→Databricks | high |
| AMZN | Amazon.com Inc | Andreessen Horowitz→Databricks | medium |
| GOOGL | Alphabet Inc | Andreessen Horowitz→Databricks | medium |
| NVDA | NVIDIA Corporation | Thrive Capital→Databricks | high |
| MSFT | Microsoft Corporation | Thrive Capital→Databricks | high |
| AMZN | Amazon.com Inc | Thrive Capital→Databricks | medium |
| GOOGL | Alphabet Inc | Thrive Capital→Databricks | medium |
| NVDA | NVIDIA Corporation | MGX→Databricks | high |
| MSFT | Microsoft Corporation | MGX→Databricks | high |
| AMZN | Amazon.com Inc | MGX→Databricks | medium |
| GOOGL | Alphabet Inc | MGX→Databricks | medium |
| NVDA | NVIDIA Corporation | Blackstone→Databricks | high |
| MSFT | Microsoft Corporation | Blackstone→Databricks | high |
| AMZN | Amazon.com Inc | Blackstone→Databricks | medium |
| GOOGL | Alphabet Inc | Blackstone→Databricks | medium |
| KTOS | Kratos Defense & Security Solutions | Elad Gil→Cambridge Aerospace | low |
| AVAV | AeroVironment Inc | Elad Gil→Cambridge Aerospace | low |
| KTOS | Kratos Defense & Security Solutions | US Government→Performance Drone Works | low |
| AVAV | AeroVironment Inc | US Government→Performance Drone Works | low |
| GOOGL | Alphabet Inc | Sequoia→Form Energy | medium |
| ETN | Eaton Corporation | Sequoia→Form Energy | low |
| PWR | Quanta Services | Sequoia→Form Energy | low |
| GOOGL | Alphabet Inc | Coatue→Form Energy | medium |
| ETN | Eaton Corporation | Coatue→Form Energy | low |
| PWR | Quanta Services | Coatue→Form Energy | low |
| NVDA | NVIDIA Corporation | Coatue→Databricks | high |
| MSFT | Microsoft Corporation | Coatue→Databricks | high |
| AMZN | Amazon.com Inc | Coatue→Databricks | medium |
| GOOGL | Alphabet Inc | Coatue→Databricks | medium |
| NVDA | NVIDIA Corporation | Andreessen Horowitz→Databricks | high |
| MSFT | Microsoft Corporation | Andreessen Horowitz→Databricks | high |
| AMZN | Amazon.com Inc | Andreessen Horowitz→Databricks | medium |
| GOOGL | Alphabet Inc | Andreessen Horowitz→Databricks | medium |
| NVDA | NVIDIA Corporation | Thrive Capital→Databricks | high |
| MSFT | Microsoft Corporation | Thrive Capital→Databricks | high |
| AMZN | Amazon.com Inc | Thrive Capital→Databricks | medium |
| GOOGL | Alphabet Inc | Thrive Capital→Databricks | medium |
| NVDA | NVIDIA Corporation | MGX→Databricks | high |
| MSFT | Microsoft Corporation | MGX→Databricks | high |
| AMZN | Amazon.com Inc | MGX→Databricks | medium |
| GOOGL | Alphabet Inc | MGX→Databricks | medium |
| NVDA | NVIDIA Corporation | Blackstone→Databricks | high |
| MSFT | Microsoft Corporation | Blackstone→Databricks | high |
| AMZN | Amazon.com Inc | Blackstone→Databricks | medium |
| GOOGL | Alphabet Inc | Blackstone→Databricks | medium |
| KTOS | Kratos Defense & Security Solutions | Elad Gil→Cambridge Aerospace | low |
| AVAV | AeroVironment Inc | Elad Gil→Cambridge Aerospace | low |
| KTOS | Kratos Defense & Security Solutions | US Government→Performance Drone Works | low |
| AVAV | AeroVironment Inc | US Government→Performance Drone Works | low |
| GOOGL | Alphabet Inc | Sequoia→Form Energy | medium |
| ETN | Eaton Corporation | Sequoia→Form Energy | low |
| PWR | Quanta Services | Sequoia→Form Energy | low |
| GOOGL | Alphabet Inc | Coatue→Form Energy | medium |
| ETN | Eaton Corporation | Coatue→Form Energy | low |
| PWR | Quanta Services | Coatue→Form Energy | low |
| NVDA | NVIDIA Corporation | Coatue→Databricks | high |
| MSFT | Microsoft Corporation | Coatue→Databricks | high |
| AMZN | Amazon.com Inc | Coatue→Databricks | medium |
| GOOGL | Alphabet Inc | Coatue→Databricks | medium |
| NVDA | NVIDIA Corporation | Andreessen Horowitz→Databricks | high |
| MSFT | Microsoft Corporation | Andreessen Horowitz→Databricks | high |
| AMZN | Amazon.com Inc | Andreessen Horowitz→Databricks | medium |
| GOOGL | Alphabet Inc | Andreessen Horowitz→Databricks | medium |
| NVDA | NVIDIA Corporation | Thrive Capital→Databricks | high |
| MSFT | Microsoft Corporation | Thrive Capital→Databricks | high |
| AMZN | Amazon.com Inc | Thrive Capital→Databricks | medium |
| GOOGL | Alphabet Inc | Thrive Capital→Databricks | medium |
| NVDA | NVIDIA Corporation | MGX→Databricks | high |
| MSFT | Microsoft Corporation | MGX→Databricks | high |
| AMZN | Amazon.com Inc | MGX→Databricks | medium |
| GOOGL | Alphabet Inc | MGX→Databricks | medium |
| NVDA | NVIDIA Corporation | Blackstone→Databricks | high |
| MSFT | Microsoft Corporation | Blackstone→Databricks | high |
| AMZN | Amazon.com Inc | Blackstone→Databricks | medium |
| GOOGL | Alphabet Inc | Blackstone→Databricks | medium |
| KTOS | Kratos Defense & Security Solutions | Elad Gil→Cambridge Aerospace | low |
| AVAV | AeroVironment Inc | Elad Gil→Cambridge Aerospace | low |
| KTOS | Kratos Defense & Security Solutions | US Government→Performance Drone Works | low |
| AVAV | AeroVironment Inc | US Government→Performance Drone Works | low |
| GOOGL | Alphabet Inc | Sequoia→Form Energy | medium |
| ETN | Eaton Corporation | Sequoia→Form Energy | low |
| PWR | Quanta Services | Sequoia→Form Energy | low |
| GOOGL | Alphabet Inc | Coatue→Form Energy | medium |
| ETN | Eaton Corporation | Coatue→Form Energy | low |
| PWR | Quanta Services | Coatue→Form Energy | low |
| NVDA | NVIDIA Corporation | Coatue→Databricks | high |
| MSFT | Microsoft Corporation | Coatue→Databricks | high |
| AMZN | Amazon.com Inc | Coatue→Databricks | medium |
| GOOGL | Alphabet Inc | Coatue→Databricks | medium |
| NVDA | NVIDIA Corporation | Andreessen Horowitz→Databricks | high |
| MSFT | Microsoft Corporation | Andreessen Horowitz→Databricks | high |
| AMZN | Amazon.com Inc | Andreessen Horowitz→Databricks | medium |
| GOOGL | Alphabet Inc | Andreessen Horowitz→Databricks | medium |
| NVDA | NVIDIA Corporation | Thrive Capital→Databricks | high |
| MSFT | Microsoft Corporation | Thrive Capital→Databricks | high |
| AMZN | Amazon.com Inc | Thrive Capital→Databricks | medium |
| GOOGL | Alphabet Inc | Thrive Capital→Databricks | medium |
| NVDA | NVIDIA Corporation | MGX→Databricks | high |
| MSFT | Microsoft Corporation | MGX→Databricks | high |
| AMZN | Amazon.com Inc | MGX→Databricks | medium |
| GOOGL | Alphabet Inc | MGX→Databricks | medium |
| NVDA | NVIDIA Corporation | Blackstone→Databricks | high |
| MSFT | Microsoft Corporation | Blackstone→Databricks | high |
| AMZN | Amazon.com Inc | Blackstone→Databricks | medium |
| GOOGL | Alphabet Inc | Blackstone→Databricks | medium |
| KTOS | Kratos Defense & Security Solutions | Elad Gil→Cambridge Aerospace | low |
| AVAV | AeroVironment Inc | Elad Gil→Cambridge Aerospace | low |
| KTOS | Kratos Defense & Security Solutions | US Government→Performance Drone Works | low |
| AVAV | AeroVironment Inc | US Government→Performance Drone Works | low |

## Candidates to watch (1)
- Palmer Luckey → Erebor (defense-tech, equity)

## Coverage — 12/48 key & core allocators produced events
_Silent this run (36) — verify these are genuinely quiet, not missed:_

- **alt_manager**: Apollo, BlackRock, DigitalBridge, Goldman Sachs, SoftBank
- **corporate**: Alphabet, Amazon, Meta, Microsoft, Oracle
- **individual**: Alexandr Wang, Austin Russell, Daniel Gross, David Sacks, Delian Asparouhov, Dylan Field, Elon Musk, Joe Lonsdale, Keith Rabois, Laura Deming, Lucy Guo, Luke Nosek, Max Levchin, Nat Friedman, Peter Thiel, Reid Hoffman, Roelof Botha, Trae Stephens, Eric Schmidt, Jeff Bezos, Marc Andreessen, Masayoshi Son, Sam Altman, Vinod Khosla
- **sovereign**: Mubadala
- **vc**: Founders Fund

## Discovered allocators — promote to watchlist? (9)
- **Clearlake Capital** (alt_manager) — seen with MGX;Blackstone;Coatue (Databricks $5B round. 2026-08-13)
- **D1 Capital Partners** (alt_manager) — seen with Altimeter (this week); Sequoia/Coatue (Anthropic Series H, prior weeks). Repeat co-investor: new participant in Thrive Holdings' $2B round (w/ Altimeter, now closed) this window, and previously named among Anthropic's Series H co-investors alongside Sequoia/Coatue/Altimeter (per W33's candidate notes). Recurs across successive weeks in watchlist-adjacent mega-deals.
- **Energy Capital Partners** (alt_manager) — seen with KKR. Co-lead (with KKR) on the ~$7.7bn (£5.75bn) agreed take-private of DCC Energy plc (LSE:DCC, FTSE 100 LPG/fuel-oil energy distributor), announced 2026-07-27 - KKR's largest European public-to-private in over a decade. Not logged as an event row this week: the target is traditional energy distribution (LPG, heating oil), not power generation/datacenter-adjacent infrastructure, so it falls outside this class's AI-buildout thesis (see summary.md). ECP is a specialist energy-transition-infrastructure manager (also holds Grain LNG in the UK) and is not yet on the watchlist; flagging since it paired with a tracked name on a large energy platform deal and could recur.
- **GIC** (sovereign) — seen with MGX;Temasek;Coatue;Blackstone (Databricks $5B round. 2026-08-13)
- **Point72** (individual) — seen with MGX;Blackstone;Coatue (Databricks $5B round. 2026-08-13)
- **Sixth Street Growth** (alt_manager) — seen with MGX;Blackstone;Coatue;T. Rowe Price (Databricks $5B round. 2026-08-13)
- **T. Rowe Price** (alt_manager) — seen with MGX;Blackstone;Coatue;Sixth Street Growth (Databricks $5B round. 2026-08-13)
- **TPG** (alt_manager) — seen with MGX;Blackstone;Coatue (Databricks $5B round. 2026-08-13)
- **Temasek** (sovereign) — seen with MGX;GIC;Coatue;Blackstone (Databricks $5B round. 2026-08-13)
